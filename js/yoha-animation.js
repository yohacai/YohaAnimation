...
/***/ function(module, exports) {
	...
	var $animateQueue = [];
	var frame_duration = requestAnimationFrame ? 4 : 16;
	...
	var requestAnimationFrame = requestAnimationFrame || function (action) {
	    setTimeout(function () {
	        action();
	    }, frame_duration);
	};
	Yoha.animate = function (el, option, duration, cb) {
	    ...
	    empty.done = function (cb) {
	        if (isEmptyObj(option)) cb && cb.call(el);else animate_task.cb = cb;
	    };
	    return empty;
	};
	            ...
	            for (var key in option) {
	                ...
	                if (Math.abs(frame[key]) > Math.abs(pattern - parseFloat(option[key]))) {
	                    for (var _key in option) {
	                        if (window_styles.indexOf(_key) < 0) style[_key] = option[_key];else el[_key] = option[_key];
	                    }
	                    ...
	                    if (window_styles.indexOf(key) < 0) style[key] = task[key] = pattern;else el[key] = task[key] = pattern;
	                }
	            }
	        });
